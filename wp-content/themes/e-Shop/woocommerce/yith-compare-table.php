...
<div id="yith-woocompare" class="woocommerce">
	<?php
        if ( empty( $products ) ):
            echo '<div class="woocommerce-error" role="alert">
										<h4>Жодного продукту не додано для порівняння.</h4>
									</div>';
        else:
    ?>
            ...
            <tr class="remove">
                ...
                foreach( $products as $product_id => $product ) :
                    ...
                    <td class="<?php echo $product_class; ?>">
                        <a href="<?php echo $yith_woocompare->obj->remove_product_url( $product_id ) ?>" data-iframe="<?php echo $iframe ?>" data-product_id="<?php echo $product_id; ?>"><span class="remove">x</span>Видалити</a>
                    </td>
                    <?php
	                ++$index;
                endforeach;
            ...
            <?php foreach ( $fields as $field => $name ) : ?>
                ...
                <tr class="<?php echo ! in_array( $field, $different ) ? $field : $field . ' different' ?>">
                    ...
                    foreach( $products as $product_id => $product ) :
                        ...
                        <td class="<?php echo $product_class; ?>"><div class="height"><?php
                            switch( $field ) {

                                case 'product_info':

                                    if( ! $fixed )
                                        echo '<div class="remove"><a href="'. $yith_woocompare->obj->remove_product_url( $product_id ) . '" data-iframe="'. $iframe . '" data-product_id="'. $product_id . '"><span class="remove">'. apply_filters('yith_woocompare_remove_icon','x') .'</span>Видалити</a></div>';
                                ...
                                default:
                                    ...
                                    if( $product instanceof WC_Product_Variation && apply_filters('yith_woocompare_support_show_single_variations',true)){
                                        ...
                                        if( strpos( $field,'pa_' ) !== false ){
                                            ...
                                            echo apply_filters( 'yith_woocompare_single_variation_field_value', do_shortcode($product->fields[$field]),$product,$field);
                                        }
                                    }else{
    ...
    <div class="row">
      <div class="col-md-3"><?php get_template_part( 'template-parts/category' ); ?>
      </div>
