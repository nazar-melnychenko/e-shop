			...
			wp_localize_script(
				...
				apply_filters(
					...
					array(
						...
						'main_product'                    => apply_filters( 'yith_wcqv_main_product_selector', 'li.product' ),
						'main_product_link'               => apply_filters( 'yith_wcqv_main_product_link_selector', 'li.product > a' ),
		...
		public function yith_quick_view_action_template() {
			...
			if ( get_option( 'yith-wcqv-product-show-add-to-cart', 'yes' ) === 'yes' ) {
				...
				if ( function_exists( 'YITH_WOCC_Frontend' ) ) {
					add_action( 'yith_wcqv_product_summary', array( YITH_WOCC_Frontend(), 'add_button' ), 22 );
				}
			}
